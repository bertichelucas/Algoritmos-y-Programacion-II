...
static void pruebas_crear_abb_vacio(){
    abb_t* abb = abb_crear(strcmp, NULL);
    print_test("Prueba abb crear abb vacio", abb);
    print_test("Prueba abb la cantidad de elementos en 0", abb_cantidad(abb) == 0);
    print_test("Prueba abb obrener clave A, es NULL, no existe", !abb_obtener(abb, "A"));
...
// Las 2 siguientes funciones son para generar cadenas aleatorias y que el arbol no quede como una lista.
...
static void prueba_abb_volumen(size_t largo, bool debug){
    ...
    char (*claves)[largo_clave] = calloc(largo * largo_clave, sizeof(char));
...
//Funcion que suma dato a extra
bool sumar(const char *clave, void* dato, void* extra){
    *(int*)extra += *(int*) dato;
    return true;
}
//Funcion que  suma dato a extra, si dato es impar no lo suma
bool sumar_acotado(const char*clave, void* dato, void* extra){
    if ((*(int*)dato) % 2 != 0) {
        return false;
    }
    *(int*)extra += *(int*) dato;
...
// Funcion que actualiza dato a 0
bool actualizar_a_ceros(const char *clave, void* dato, void* extra){
    // Tuve que agregar esta linea porque el compilador de la catedra
    // tiraba un warning que no se usaba extra
    if (extra == NULL) { 
        *(int*)dato = 0;
    }
    return true;
}
// Funcion que recibe claves numericas en forma de cadena y devuelve cual es mayor
...
static void prueba_abb_iterar_ext_correcto_orden(){
    // Verifica que el iterador externo recorra in-order
    abb_t* abb = abb_crear(numcmp, NULL);
    char *clave1 = "5", *clave2 = "2", *clave3 = "8", *clave4 = "3";
    int a = 3, b = 2, c = 2, d = 4, i = 0;
    int datos[4];
    abb_guardar(abb, clave1, &a);
    abb_guardar(abb, clave2, &b);
    abb_guardar(abb, clave3, &c);
    abb_guardar(abb, clave4, &d);

    abb_iter_t *iter = abb_iter_in_crear(abb);
    print_test("iterador no al final", !abb_iter_in_al_final(iter));
    while (!abb_iter_in_al_final(iter)) {
        const char* actual = abb_iter_in_ver_actual(iter);
        datos[i++] = *(int*) abb_obtener(abb, actual);
        abb_iter_in_avanzar(iter);
    }
    bool ok;
    ok = datos[0] == b && datos[1] == d && datos[2] == a &&  datos[3] == c;
    print_test("Recorido in order correcto", ok);
    abb_iter_in_destruir(iter);
    abb_destruir(abb);

}

static void prueba_abb_iter_inetrno(){
    abb_t* abb = abb_crear(numcmp, NULL);
    char *clave1 = "5", *clave2 = "2", *clave3 = "8", *clave4 = "3";
    int a = 3, b = 2, c = 2, d = 4;
    ...
    print_test("la suma total calculada con el iterador interno es 11", suma == 11);
    suma = 0;

    abb_in_order(abb, sumar_acotado, &suma);
    print_test("la suma hasta encontrar el primer numero impar es 6", suma == 6);
    
    abb_in_order(abb, actualizar_a_ceros, NULL);
    suma = 0;

    abb_in_order(abb, sumar, &suma);    
    print_test("la suma total luego de actualizar todos los elementos a 0 es 0", suma == 0);
...
void pruebas_abb_estudiante() {
    ...
    prueba_abb_iterar_ext_correcto_orden();
    prueba_abb_iter_inetrno();
    prueba_abb_iterar();
